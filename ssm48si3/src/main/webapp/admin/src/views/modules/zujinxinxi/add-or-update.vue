<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="房东账号" prop="fangdongzhanghao">
          <el-select  @change="fangdongzhanghaoChange" v-model="ruleForm.fangdongzhanghao" placeholder="请选择房东账号">
            <el-option
                v-for="(item,index) in fangdongzhanghaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.fangdongzhanghao" label="房东账号" prop="fangdongzhanghao">
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.zukexingming!=''&&json.zukexingming){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/fangdong/fangdongzhanghao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.fangdongzhanghaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    fangwumingchengChange () {
      this.$http({
        ...
        if (data && data.code === 0) {
              if(data.data.fangwuleixing){
                this.ruleForm.fangwuleixing = data.data.fangwuleixing
              }
              if(data.data.suozaiquyu){
                this.ruleForm.suozaiquyu = data.data.suozaiquyu
              }
              ...
              if(data.data.fangdongxingming){
                this.ruleForm.fangdongxingming = data.data.fangdongxingming
              }
        } else {
          this.$message.error(data.msg);
        }
    ...
    // 下二随
    fangdongzhanghaoChange () {
      this.$http({
        url: `follow/fangdong/fangdongzhanghao?columnValue=`+ this.ruleForm.fangdongzhanghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.fangwuleixing){
