<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="房屋名称" prop="fangwumingcheng">
          <el-select  @change="fangwumingchengChange" v-model="ruleForm.fangwumingcheng" placeholder="请选择房屋名称">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.fangwumingcheng" label="房屋名称" prop="fangwumingcheng">
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.zulinpingzheng" label="租赁凭证" prop="zulinpingzheng">
          <file-upload
          tip="点击上传租赁凭证"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.zulinpingzheng?ruleForm.zulinpingzheng:''"
          @change="zulinpingzhengUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.zulinpingzheng" label="租赁凭证" prop="zulinpingzheng">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.zulinpingzheng.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.qiandingshijian = true;
          ...
          if(o=='zulinpingzheng'){
            this.ruleForm.zulinpingzheng = obj[o];
	    this.ro.zulinpingzheng = true;
            continue;
          }
          if(o=='userid'){
    ...
    fangwumingchengChange () {
      this.$http({
        url: `follow/fangwuxinxi/fangwumingcheng?columnValue=`+ this.ruleForm.fangwumingcheng,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.zhuangtai){
                this.ruleForm.zhuangtai = data.data.zhuangtai
              }
              if(data.data.zujin){
                this.ruleForm.zujin = data.data.zujin
              }
              if(data.data.xiangxidizhi){
                this.ruleForm.xiangxidizhi = data.data.xiangxidizhi
              }
              if(data.data.zukexingming){
                this.ruleForm.zukexingming = data.data.zukexingming
              }
              if(data.data.shouji){
    ...
    yonghumingChange () {
      this.$http({
        url: `follow/zuke/yonghuming?columnValue=`+ this.ruleForm.yonghuming,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.zhuangtai){
    ...
    zulinpingzhengUploadChange(fileUrls) {
	this.ruleForm.zulinpingzheng = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
